...
class Logger:
    def __init__(self):
        logging.basicConfig(filename='mfwk_debug.log', encoding='UTF-8', level=logging.DEBUG, format='%(asctime)s [%(name)s] %(message)s')
    ...
    def logging(self):
        '''Reusable logger'''
        _ = str(datetime.today().strftime("%d-%m-%Y_%H-%M-%S"))
        g0 = self.RGB()
